<script lang="ts">
	import type { StarConfig } from '../types.js';

	export let id: string;
	export let readOnly: boolean;
	export let fillPercentage: number;
	export let starConfig: Required<StarConfig>;
</script>
		...
		<defs>
			<linearGradient id="linear-gradient-{id}" x1="0%" y1="0%" x2="100%" y2="0%">
				...
				<stop
					...
					style="stop-color:{starConfig.fillColor};stop-opacity:1"
				/>
				<stop
					offset="{fillPercentage * 100}%"
					style="stop-color:{starConfig.unfilledColor};stop-opacity:1"
				/>
			</linearGradient>
		</defs>
	{/if}
